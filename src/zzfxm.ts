...
const zzfxM = (instruments: IInstrument[], sequence: number[], patterns: { [key: number]: IPattern[]}, BPM: number = 125) => {
  ...
  for(; hasMore; channelIndex++) {
    ...
    sequence.map((patternIndex: number, sequenceIndex: number) => {
      ...
      const pat = !!patterns[patternIndex][channelIndex] as unknown;
      hasMore |= pat as number;
